...
export class EmailService {
  ...
  constructor() {
    ...
    this.fromAddress = process.env.RESEND_FROM_EMAIL || 'BayReady <notifications@resend.dev>';
  ...
  async sendBookingConfirmation(params: {
    ...
    try {
      await this.resend.emails.send({
        from: `${merchantName} via BayReady <${this.fromAddress.match(/<(.+)>/)?.[1] || 'notifications@resend.dev'}>`,
        ...
        html: `
          <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 500px; margin: 0 auto; padding: 24px;">
            ...
            <p style="color: #aaa; font-size: 11px; margin: 0;">Powered by BayReady</p>
          </div>
        `,
      });
